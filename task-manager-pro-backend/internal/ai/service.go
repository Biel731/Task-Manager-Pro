...
func (s *Service) GenerateSubtasks(ctx context.Context, title, description string) ([]SubtaskDTO, error) {
	t := strings.TrimSpace(title)
	d := strings.TrimSpace(description)
	...
	subtasks, err := s.provider.GenerateSubtasks(ctx, t, d)
	if err != nil {
		return nil, err
	}

	// normaliza e limita
	clean := make([]SubtaskDTO, 0, 10)
	seen := map[string]struct{}{}

	for _, st := range subtasks {
		name := strings.TrimSpace(st.Title)
		if name == "" {
			continue
		}
		if len(name) > 120 {
		...
		seen[key] = struct{}{}
		clean = append(clean, SubtaskDTO{Title: name, Done: false})
		if len(clean) == 10 {
			break
		}
	}
	...
	// fallback mínimo
	for len(clean) < 5 {
		clean = append(clean, SubtaskDTO{Title: "Definir próximo passo", Done: false})
	}
