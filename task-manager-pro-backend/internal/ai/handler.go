...
func (h *Handler) GenerateSubtasks(c *gin.Context) {
	start := time.Now()
	userID, _ := auth.GetUserID(c)

	var req GenerateSubtasksRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		log.Printf("[AI] user=%d endpoint=/ai/generate-subtasks status=400 ms=%d err=bind_json",
			userID, time.Since(start).Milliseconds(),
		)
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid json"})
		return
	...
	if msg := req.Validate(); msg != "" {
		log.Printf("[AI] user=%d endpoint=/ai/generate-subtasks status=400 ms=%d err=%s",
			userID, time.Since(start).Milliseconds(), msg,
		)
		c.JSON(http.StatusBadRequest, gin.H{"error": msg})
		return
	...
	subtasks, err := h.service.GenerateSubtasks(c.Request.Context(), req.Title, req.Description)
	if err != nil {
		log.Printf("[AI] user=%d endpoint=/ai/generate-subtasks status=500 ms=%d err=%v",
			userID, time.Since(start).Milliseconds(), err,
		)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to generate subtasks"})
		return
	}

	log.Printf("[AI] user=%d endpoint=/ai/generate-subtasks status=200 ms=%d",
		userID, time.Since(start).Milliseconds(),
	)

	c.JSON(http.StatusOK, GenerateSubtasksResponse{Subtasks: subtasks})
}
